...
class ToolPassTests(unittest.TestCase):
    ...
    def test_plain_model_completion_without_receipt_is_rejected(self):
        runner = self.runner(["Done — I updated the file."], max_steps=2)
        result = runner.run("files", "update the file")
        self.assertFalse(result.complete)
        self.assertEqual(result.verification["status"], "no_action")
        self.assertIn(INCOMPLETE_MARKER, result.report)

    def test_missing_material_input_pauses_without_acting(self):
        runner = self.runner([
            "NEED_INPUT: Which report path should I read?",
        ])
        result = runner.run("files", "read the report")
        self.assertFalse(result.complete)
        self.assertEqual(result.question, "Which report path should I read?")
        self.assertEqual(self.executor.calls, [])

    def test_file_write_requires_content_readback(self):
        self.executor.results.update({
            "write_file": "File written successfully",
            "read_file": "new setting=true",
        })
        runner = self.runner([
            '{[(( write_file(path="/tmp/config", content="new setting=true") ))]}',
            '{[(( read_file(path="/tmp/config") ))]}',
            "I updated /tmp/config and read back new setting=true.",
        ])
        result = runner.run(
            "files",
            "update /tmp/config",
            success_check="Read-back contains new setting=true",
        )
        self.assertTrue(result.complete)
        self.assertEqual(result.verification["status"], "verified")
        self.assertEqual(
            [item.tool for item in result.receipts],
...
class OrchestrationTests(unittest.TestCase):
    ...
    def test_planner_question_causes_no_tool_calls(self):
        orch = self.orchestrator(
            "ASK | Which Alex should receive the report?"
        )
        result = orch.handle("Email Alex the report")
        self.assertFalse(result.complete)
        self.assertEqual(result.question, "Which Alex should receive the report?")
        self.assertEqual(self.executor.calls, [])

    def test_unverified_first_leg_stops_downstream_action(self):
        scripts = {
            "files": [
                '{[(( write_file(path="/tmp/a", content="new") ))]}',
                "I wrote the file.",
            ],
            "web": [
                '{[(( search(query="should not run") ))]}',
                "Found it.",
            ],
        }
        ...
        runner = ToolTaskRunner(
            self.registry, self.executor, factory, max_steps=3
        )
        orch = ToolOrchestrator(
            runner,
            lambda prompt: (
                "LEG | files | Update /tmp/a to new | Read-back contains new\n"
                "LEG | web | Search for a follow-up | Search receipt contains result"
            ) if "Split this request" in prompt else "The update was not verified.",
        )
        result = orch.handle("update the file then search")
        self.assertFalse(result.complete)
        self.assertIn("write_file", [name for name, _args in self.executor.calls])
        self.assertNotIn("search", [name for name, _args in self.executor.calls])

    def test_verified_and_failed_routes_cross_experience_boundary(self):
        observed = []
        orch = self.orchestrator(
            "LEG | files | Read the notes | Read receipt contains the notes",
            experience_provider=lambda _request: "- avoid write_file after tool_failure",
            experience_observer=lambda request, sequence, verified, errors: observed.append(
                (request, sequence, verified, errors)
            ),
        )
        result = orch.handle("read my notes")
        self.assertTrue(result.complete)
        self.assertIn("avoid write_file", self.plan_prompts[0])
        self.assertEqual(observed[0][1], ["read_file"])
        self.assertTrue(observed[0][2])
        self.assertGreater(result.planning_prompt_tokens, 0)
...
class SessionWrapperTests(unittest.TestCase):
    ...
    def test_clarification_is_returned_without_claiming_completion(self):
        asking = ToolOrchestrator(
            self.runner,
            lambda _prompt: "ASK | Which notes file should I read?",
        )
        inner = self.Inner([
            '{"tool_request": "read my notes"}',
            "Which notes file should I read?",
        ])
        session = OrchestratedToolSession(inner, asking)
        reply = session.send_message("read my notes")
        self.assertEqual(reply, "Which notes file should I read?")
        self.assertIn("do not claim completion", inner.received[-1])
        self.assertEqual(self.executor.calls, [])
