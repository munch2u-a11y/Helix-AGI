...
Simple, direct belief merging:
  ...
  2. Send matches to the LLM for MERGE/PASS decision (one call per belief, parallel)
...
from llm.providers.base import AUXILIARY_MODEL, ProviderConfig, create_session
_MODEL = AUXILIARY_MODEL
_MAX_MATCH_CANDIDATES = 6   # Top N existing beliefs to send for comparison
_MAX_WORKERS = 4             # Parallel LLM calls
...
# ── LLM API Call (Agnostic) ───────────────────────────────────────────────────────

def _call_llm(prompt: str, system: str = "") -> Optional[str]:
    """Single LLM API call, using the agnostic provider interface."""
    try:
        # Use the AUXILIARY_MODEL defined in base.py
        config = ProviderConfig(
            provider_type=os.environ.get("HELIX_AUXILIARY_PROVIDER", "gemini"), # Default to gemini if not set
            model=_MODEL,
            temperature=0.15,
            max_output_tokens=512,
        )
        session = create_session(config, system_instruction=system)
        response_text = session.send_message(prompt)
        return response_text.strip()

    except Exception as e:
        logger.warning("LLM consolidation call failed: %s", e)
    return None
...
\
    # Build review prompt and call LLM\
    prompt = _build_review_prompt(new_belief, candidates)\
    raw_response = _call_llm(prompt, system=_SYSTEM_PROMPT)\
\
    if not raw_response:\
...
\
    For each new belief:\
      ...
      - If matches found: ask LLM to MERGE or PASS\
...
def _call_llm_for_relations(new_content: str, candidates: List[Dict[str, Any]]) -> List[str]:\
    """Ask LLM to disambiguate which beliefs were actually relied upon."""\
    prompt = (\
    ...
    \
    result = _call_llm(prompt, system="You return only comma-separated IDs.")\
