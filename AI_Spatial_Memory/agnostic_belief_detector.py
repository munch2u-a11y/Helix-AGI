"""
Helix — Belief Detector (Post-Pulse Hook) - LLM Agnostic
...
Architecture:
  ...
  - Uses a generic LLM provider (e.g., Ollama, Gemini) to classify whether the thought
...
# Placeholder for LLM provider and model name
...
def set_dependencies(belief_store, physics_engine, sentinel=None, llm_provider=None, llm_model: str = "generic-llm-model"):
    """Wire dependencies at startup. Called from main.py."""
    global _belief_store, _physics_engine, _sentinel, _GLOBAL_LLM_PROVIDER, _GLOBAL_LLM_MODEL
    _belief_store = belief_store
    _physics_engine = physics_engine
    _sentinel = sentinel
    _GLOBAL_LLM_PROVIDER = llm_provider
    _GLOBAL_LLM_MODEL = llm_model
    logger.info("Belief detector: dependencies wired (LLM-agnostic)")


# ── LLM Agnostic Classification ─────────────────────────────────────
...
_CLASSIFICATION_PROMPT = (
    ...
    "- A description of what happened ('I replied to Jean-Luc')\n"
    ...
    "- A relational understanding ('Jean-Luc values sovereignty in AI design')\n"
...
def _classify_thought(thought_text: str) -> Optional[Tuple[str, str]]:
    """Send thought to a generic LLM provider for belief classification.

    Returns (belief_text, category) if a realization is found, else None.
    Falls back silently if LLM provider is unavailable.
    """
    if _GLOBAL_LLM_PROVIDER is None:
        logger.warning("No LLM provider set for belief classification.")
        return None

    # Truncate very long thoughts to save LLM tokens
...
def belief_detector_hook(ctx) -> None:
    ...
    Flow:
      ...
      2. Call generic LLM to classify the thought
    ...
    if _belief_store is None or _physics_engine is None or _GLOBAL_LLM_PROVIDER is None:
        return

    # 1. Classify the thought via generic LLM
    result = _classify_thought(ctx.thought)
    if result is None:
        return  # No realization detected — most common path

    belief_text, category = result
    logger.debug("LLM detected belief candidate: [%s] %s", category, belief_text[:80])
