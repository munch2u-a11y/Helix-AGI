...
**Scope:** [`core/pulse_loop.py`](../../core/pulse_loop.py)

---

## 1. Runtime Role & State Machine

The `PulseLoop` ([`core/pulse_loop.py`](../../core/pulse_loop.py#L120-L280)) owns the event queue, LLM chat session, cadence state transitions, context compression policy, and post-pulse hook dispatch.
...
In `_pulse()` ([`core/pulse_loop.py`](../../core/pulse_loop.py#L810-L1230)):
...
4. **LLM Function Execution**: Executes model inference, parses function calls, and dispatches tools via `ToolExecutor` and [`tools/tool_registry.py`](../../tools/tool_registry.py#L30-L110).
5. **Memory Storage & Physics Step**: Persists thought snapshots to [`cognitive_journal.jsonl`](../../memory/cognitive_journal.py) and advances spatial attention coordinates ([`core/physics_engine.py`](../../core/physics_engine.py#L210-L290)).
...
When tool calls fail during execution, `_pulse()` notifies `ToolLessonTracker` ([`core/tool_lesson_tracker.py`](../../core/tool_lesson_tracker.py#L20-L90)). Operational lessons are captured, deduplicated, and queued for nightly Curator consolidation ($G=2.5$), crystallizing into `skills` beliefs for future sessions.
