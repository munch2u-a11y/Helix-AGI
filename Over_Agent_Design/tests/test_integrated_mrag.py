...
class TestEmbeddedMRAG(unittest.TestCase):
    def test_real_semantic_lane_prioritizes_layer2_term(self):
        ...
        with tempfile.TemporaryDirectory(prefix="helix_overagent_mrag_") as tmp:
            ...
            fixtures = {
                ...
                "concepts": ("provenance", "Provenance links derived beliefs to source evidence."),
            ...
            # Provider-free test: exact Layer-2 lexicon priority is part of
            # the semantic lane and does not require a live embedding model.
            ...
            context = runtime.recall_context("What does Nemo build?", top_k=3)
            status = runtime.get_status()

            self.assertTrue(runtime.last_retrieval)
            self.assertEqual(runtime.last_retrieval[0]["id"], "people_fixture")
            self.assertEqual(runtime.last_retrieval[0]["tier"], 2)
            self.assertEqual(status["retrieval_mode"], "main_semantic_384d_bridge")
            self.assertEqual(status["semantic_dimensions"], 384)
            ...
            memory_id = runtime.remember(
                "A compatibility receipt was verified.",
                tags=["over_agent"],
                record_metadata={
                    "record_kind": "action_receipt",
                    "evidence_scopes": ["tool"],
                },
            )
            stored = runtime.memory_manager.get_recent(limit=1)[0]
            self.assertEqual(memory_id, stored["id"])
            self.assertIn("record:record_kind:action_receipt", stored["tags"])
            self.assertIn("record:evidence_scopes:tool", stored["tags"])
            runtime.close()
    ...
    def test_document_ingester_uses_mrag_write_boundary(self):
        ...
        recorder = RecordingMRAG()
        ingester = DocumentIngester(
            chunk_size_words=50,
            overlap_words=10,
            mrag_runtime=recorder,
        )
...
class _RecordingMRAG:
    ...
    def recall_context(self, query, top_k=5):
...
class TestConductorMRAGFlow(unittest.TestCase):
    def test_turn_recalls_then_records_inbound_and_outbound(self):
        ...
        self.assertEqual(mrag.memories[0][1]["source"], "pulse_input")
        self.assertEqual(mrag.memories[1][1]["source"], "helix_outbound")
        self.assertTrue(mrag.memories[1][1]["persist_index"])
        self.assertFalse(any(
            "mRAG RECALLED CONTEXT" in event["content"]
