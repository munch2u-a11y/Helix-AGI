...
class ToolExecutor:
    ...
    def _populate_registry(self):
        ...
        from tools.tool_declarations import (
            ...
            PERCEPTION_TOOLS, WEB_TOOLS, FILES_TOOLS,
        ...
        def _check_camera_enabled():
            try:
                from core.pulse_loop import PulseLoop
                cfg = PulseLoop._load_config()
                if not cfg.get("camera_enabled", True):
                    return False
            except Exception:
                pass
            import glob
        ...
        def _check_ptz_enabled():
            if not _check_camera_enabled():
                return False
            try:
                from core.pulse_loop import PulseLoop
                cfg = PulseLoop._load_config()
                return bool(cfg.get("ptz_enabled", False))
            except Exception:
                return False

        def _check_mic_enabled():
            try:
                from core.pulse_loop import PulseLoop
                cfg = PulseLoop._load_config()
                return bool(cfg.get("mic_enabled", True))
            except Exception:
                return True
        ...
        registry.register_toolset_description("perception", "Physical senses — camera and microphone")
        for tool in PERCEPTION_TOOLS:
            name = tool["name"]
            handler = perception_handlers.get(name)
            if handler:
                chk = None
                if name in ("look", "record_video"):
                    chk = _check_camera_enabled
                elif name in ("ptz_look", "camera_auto_track"):
                    chk = _check_ptz_enabled
                elif name == "listen":
                    chk = _check_mic_enabled
                registry.register(
                    name=name,
                    toolset="perception",
                    schema=tool,
    ...
    def get_registered_tool_names(self) -> List[str]:
        """Return a list of all registered tool names in the registry and dispatch fallback."""
        names = []
        if hasattr(self, "_registry") and self._registry:
            try:
                names.extend(list(self._registry.entries.keys()))
            except Exception:
                pass
        if hasattr(self, "_FC_DISPATCH") and self._FC_DISPATCH:
            names.extend(list(self._FC_DISPATCH.keys()))
    ...
    def execute_function_call(self, name: str, args: dict) -> str:
        ...
        try:
            ...
            # Tool learning boundary: failures are captured as lesson
            # candidates, successes verify recently injected lessons.
            try:
                from core.tool_lesson_tracker import observe_tool_result
                observe_tool_result(name, args, result or "")
            except Exception as e:
                logger.debug(f"Tool lesson observation failed: {e}")

            # Interaction provenance: record what was responded to, and
    ...
    # Chunk size for read_file. Bounded chunks keep any single tool
    # result from flooding the context window (a 1.9MB world_state.json
    # once wedged the whole session), and give the preconscious a
    # digestible unit to scan for familiar entities per chunk.
    READ_CHUNK_CHARS = 24_000
    ...
    def _fc_read_file(self, args: dict) -> str:
        ...
        try:
            ...
            total = len(content)
            if offset >= total > 0:
                return f"[{path}: offset {offset} is past end of file ({total} chars)]"

            chunk = content[offset:offset + self.READ_CHUNK_CHARS]
            end = offset + len(chunk)

            if offset == 0 and end >= total:
                return content  # small file — no chunk framing needed

            header = f"[{path} — chars {offset}-{end} of {total}"
            if end < total:
                header += f" | {total - end} remaining, continue with offset={end}"
            header += "]\n"
            return header + chunk
        except Exception as e:
    ...
    def _fc_enable_toolset(self, args: dict) -> str:
        ...
        if self._pulse_loop and hasattr(self._pulse_loop, '_active_toolsets'):
            ...
            # Record it as active at current turn to start tracking
            if hasattr(self, '_registry') and hasattr(self._pulse_loop, '_pulse_turn_counter'):
                self._registry.record_toolset_active(toolset_name, self._pulse_loop._pulse_turn_counter)
    ...
    def _fc_disable_toolset(self, args: dict) -> str:
        ...
        if self._pulse_loop and hasattr(self._pulse_loop, '_active_toolsets'):
            ...
            # Deactivate tracking
            if hasattr(self, '_registry'):
                self._registry.deactivate_toolset_tracking(toolset_name)
