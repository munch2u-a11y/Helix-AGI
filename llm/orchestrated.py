...
class OrchestratedToolSession(ChatSession):
    ...
    def send_message(self, message: str) -> str:
        ...
        for _ in range(MAX_REQUESTS_PER_TURN):
            ...
            self._record(result)
            if getattr(result, "question", ""):
                thought = self._inner.send_message(
                    "The action is paused before any further work. Ask the person "
                    "this exact clarification question and do not claim completion:\n"
                    + result.question
                )
            elif result.complete:
                thought = self._inner.send_message(
                    f"{result.reply}\n\n"
                    "The system verified that outcome from tool receipts. Now give your reply."
                )
            else:
                thought = self._inner.send_message(
                    f"{result.reply}\n\n"
                    "The attempted action was not fully verified. Report only what the "
                    "receipts establish and say what remains; do not claim completion."
                )
    ...
    def _record(self, result) -> None:
        for observation in result.observations:
            self._pending_tool_results.append({
                ...
                "receipt": getattr(observation, "receipt", {}) or {},
            })
...
def build_orchestrator(
    ...
    def plan_llm(prompt: str) -> str:
        ...
        try:
            output = pass_session.send(prompt)
            try:
                plan_llm.total_model_tokens += int(
                    pass_session.get_last_token_count() or 0
                )
            except Exception:
                pass
            return output
    ...
    plan_llm.total_model_tokens = 0

    experience_provider, experience_observer = _procedural_experience(data_dir)
    return ToolOrchestrator(
        ...
        experience_observer=experience_observer,
    )


def _procedural_experience(data_dir: str):
    """Bridge verified local runs into the shared compact procedure store."""
    from core.task_cognition.models import TaskRecord
    from core.task_cognition.procedures import ProceduralMemory

    memory = ProceduralMemory(Path(data_dir) / "tasks")

    def task_for(request: str) -> TaskRecord:
        return TaskRecord(objective=request, task_type="action")

    def provider(request: str) -> str:
        lines = []
        for item in memory.relevant(task_for(request), limit=3):
            sequence = " -> ".join(item.get("tool_sequence", []))
            if not sequence:
                continue
            if item.get("recommended"):
                lines.append(
                    f"- prefer {sequence} (verified reliability "
                    f"{float(item.get('reliability', 0.0)):.2f})"
                )
            elif item.get("avoid"):
                errors = ", ".join(sorted(item.get("error_codes", {})))
                suffix = f"; errors: {errors}" if errors else ""
                lines.append(f"- avoid or repair {sequence}{suffix}")
        return "\n".join(lines)
